...
class EpicenterEngine {
 public:
  ...
  void processPcm16(const int16_t* in, int16_t* out, int frameCount, int channelCount) {
    ...
    ensureSubBufferCapacity(frameCount);
  ...
  void processFloat(const float* in, float* out, int frameCount, int channelCount) {
    ...
    for (int i = 0; i < frameCount; ++i) {
      const float left = clampf(in[i * usedChannels], -1.0f, 1.0f);
      const float right = usedChannels > 1 ? clampf(in[i * usedChannels + 1], -1.0f, 1.0f) : left;
      processDetectorSample(i, left, right, gateHoldSamples, synthAmount);
    }

    for (int i = 0; i < frameCount; ++i) {
      for (int ch = 0; ch < usedChannels; ++ch) {
        ChannelState& state = channels_[std::min(ch, channelCount_ - 1)];
        const float sample = denormalFloor(clampf(in[i * usedChannels + ch], -1.0f, 1.0f));
        out[i * usedChannels + ch] = processChannelSample(state, sample, intensityNorm, bassProgramAmount, lowMidBodyAmount, lowMidDipAmount, volumeGain, subBuffer_[static_cast<size_t>(i)]);
      }
    }
  }
  ...
  void resetState() {
    for (auto& c : channels_) {
      c.voiceHighpass.reset();
      c.bassLowpass.reset();
      c.lowMidBody.reset();
      c.lowMidDip.reset();
      c.subLowpass.reset();
      c.outputDcHighpass.reset();
      c.voiceEnv.reset();
    }

    monoState_.band60.reset();
    monoState_.band80.reset();
    monoState_.band110.reset();
    monoState_.monoLowpass.reset();
    monoState_.diffHighpass.reset();
    monoState_.synthHighpass.reset();
    monoState_.synthLowpass.reset();
    monoState_.detectorEnv.reset();
    monoState_.monoEnv.reset();
    monoState_.diffEnv.reset();
    monoState_.gateEnv.reset();
    monoState_.synthLevelEnv.reset();
    monoState_.lastDetector = 0.0f;
    monoState_.flipState = 1;
    monoState_.holdSamples = 0;
 ...
 private:
  ...
  void ensureSubBufferCapacity(int frameCount) {
    const size_t required = static_cast<size_t>(std::max(0, frameCount));
    if (subBuffer_.size() < required) {
      subBuffer_.resize(required);
    }
  }

  void processDetectorSample(int index, float left, float right, int gateHoldSamples, float synthAmount) {
    const float mono = denormalFloor((left + right) * 0.5f);
    const float diff = denormalFloor((left - right) * 0.5f);
    ...
    subBuffer_[static_cast<size_t>(index)] = denormalFloor(protectedSynth * synthAmount * remixGate);
  }

  float processChannelSample(
    ChannelState& state,
    float sample,
    float intensityNorm,
    float bassProgramAmount,
    float lowMidBodyAmount,
    float lowMidDipAmount,
    float volumeGain,
    float generatedSubSource
  ) {
    const float voicePath = state.voiceHighpass.process(sample);
    const float voicePresence = state.voiceEnv.process(voicePath);
    const float voiceProtection = std::max(0.5f, 1.0f - voicePresence * (0.85f + intensityNorm * 0.3f));
    ...
    const float generatedSub = state.subLowpass.process(generatedSubSource) * (0.4f + voiceProtection * 0.6f);
